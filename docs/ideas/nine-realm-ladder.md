...
**Inner Tempering** has a saturation limit (Late = full mortal frame). Diminishing returns only apply while filling it. Peak is optional and still mortal. Lifespan stays the 80-year cap. The physique is built from what the flesh kept; geniuses are fast and clean, not a second spirit root. See [`body-path-refining-rewrite.md`](body-path-refining-rewrite.md).
